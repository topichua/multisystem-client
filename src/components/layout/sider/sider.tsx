...
const MenuSiderMain = observer(({ showLabel, onOpenDrawer, onCloseDrawer }: MenuSiderMainProps) => {
  ...
  const renderNavButton = (item: SiderNavItem) => {
    const isActive = item.isActive?.(location.pathname) ?? location.pathname === item.path;
    ...
    return (
      <S.Button
        ...
        block
        style={showLabel ? { display: 'flex', justifyContent: 'flex-start' } : undefined}
        data-qa={showLabel ? `fullWidth_menu_sider_${qaKey}` : `menu_sider_${qaKey}`}
        $showLabel={showLabel}
      >
        {showLabel ? <Text className="common-button-text">{label}</Text> : undefined}
      </S.Button>
    );
  };

  return <S.MenuSider $showLabel={showLabel}>{siderNavItems.map(renderNavButton)}</S.MenuSider>;
});
    ...
    <S.Wrapper>
      ...
      >
        <S.SiderScrollArea>
          ...
          >
            <S.BrandLogo src="/logos/only_icon_logo.svg" alt={`${t('brand')} icon`} />
