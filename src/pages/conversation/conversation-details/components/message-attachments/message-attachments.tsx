...
const getAttachmentImageUrl = (attachment: MessageAttachmentEntry) => {
  return attachment.image_data?.url ?? attachment.image_data?.preview_url ?? null;
};

export const MessageAttachments = memo(({ messageId, attachments }: MessageAttachmentsProps) => {
  ...
  return (
    <S.Attachments>
      {imageAttachments.map((attachment) => (
        <Image width={200} key={attachment.id} alt="Message attachment" src={attachment.imageUrl} />
      ))}
    </S.Attachments>
  );
});
