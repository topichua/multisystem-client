...
import { Avatar, Button, Flex, Form, Input, Skeleton, Spin, Typography, message } from 'antd';
...
function participantAvatarInitials(conversation: Conversation, linkedClient?: Client): string {
  if (linkedClient) {
    const s = `${linkedClient.firstName?.[0] ?? ''}${linkedClient.lastName?.[0] ?? ''}`.trim();
      ...
      conversation ? (
        <S.ParticipantPhoto $flush={flush}>
          <Avatar
            ...
            alt={conversation.participant.name || conversation.participant.username || ''}
          >
      ...
      <S.Root aria-label={t('conversation.clientPanelAria')}>
        ...
        <S.PanelScroll>
          {!conversation ? (
            ...
            showCreateForm ? (
              <Form form={form} layout="vertical" initialValues={initialCreateValues}>
                ...
                <Form.Item name="delivery_info" label={t('clients.deliveryInfo')}>
              ...
              <S.EmptyCenter>
                <Flex vertical align="center" gap={12} style={{ width: '100%', maxWidth: 300 }}>
                  ...
                  <Button type="primary" size="large" onClick={() => setShowCreateForm(true)}>
            ...
            <Text type="secondary">{t('conversation.clientRecordMissing')}</Text>
          )}
            ...
            <ClientOrderDrawer
              ...
              onOrderCreated={() => {
                ...
                if (ordersStore.clientStatsClientId != null) {
                  void ordersStore.loadClientStats(ordersStore.clientStatsClientId);
                }
                if (ordersStore.clientOrdersClientId != null) {
                  void ordersStore.loadClientOrders(ordersStore.clientOrdersClientId);
                }
              }}
            />
          ) : null}
