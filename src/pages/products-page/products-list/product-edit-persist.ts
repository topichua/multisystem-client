import type { ProductDetails, ProductUpdatePayload } from '@/features/products/model/product.types';

import type { ProductEditFormValues } from './product-modal.types';

export const buildProductUpdatePayload = (values: ProductEditFormValues): ProductUpdatePayload => ({
  name: values.name,
...
const productFieldValue = (product: ProductDetails, field: keyof ProductEditFormValues) => {
  switch (field) {
    case 'name':
      return product.name;
    case 'description':
    ...
    case 'referenceGroupId':
      return product.referenceGroupId == null ? '' : String(product.referenceGroupId);
    case 'price':
...
const formFieldValue = (values: ProductEditFormValues, field: keyof ProductEditFormValues) => {
  switch (field) {
    case 'description':
    case 'sourceId':
    case 'referenceGroupId':
...
): boolean => productFieldValue(product, field) === formFieldValue(values, field);
...
export const resolveGalleryPersistPayload = (
  ...
  const resolvedCoverId =
    items.length === 0
      ...
      : activeCoverId != null && items.some((item) => item.id === activeCoverId)
        ...
        : items[0].id;
  const coverItem =
    resolvedCoverId != null ? items.find((item) => item.id === resolvedCoverId) : null;
  ...
  const remainingGalleryUrls = items.map((item) => item.previewUrl.trim()).filter(Boolean);
