...
export const ProductCategoryDetailView = observer(() => {
  ...
  const categoryFromList = useMemo(
    () => (Number.isFinite(idNum) ? findCategoryById(store.categories, idNum) : undefined),
  ...
  const parentOptions = useMemo(
    () =>
      categoriesEligibleAsParent(store.categories, Number.isFinite(idNum) ? idNum : undefined).map(
  ...
  if (!store.listLoading && !store.detailLoading && !category) {
    return (
      <Alert
        ...
        showIcon
        action={
          <Button size="small" onClick={() => navigate(pagesMap.productsCategories)}>
        ...
        <PaneDetailLayout.Body>
          <Form form={form} layout="vertical" style={{ maxWidth: 520 }} onFinish={handleSave}>
            ...
            <Form.Item name="parentId" label={t('categories.parentCategory')}>
              <Select allowClear placeholder={t('categories.noParent')} options={parentOptions} />
            </Form.Item>
          </Form>
