...
function kebabBasename(fullBasename) {
  ...
  if (fullBasename.endsWith('.tsx')) return `${toKebabSegment(fullBasename.slice(0, -4))}.tsx`;
  if (fullBasename.endsWith('.ts')) return `${toKebabSegment(fullBasename.slice(0, -3))}.ts`;
  return fullBasename;
}
...
for (const abs of files) {
  ...
  const caseOnlyRename =
    path.basename(abs).toLowerCase() === nextBase.toLowerCase() && path.basename(abs) !== nextBase;
...
for (const { oldAbs, newAbs, caseOnlyRename } of renames) {
  ...
  if (caseOnlyRename) {
    const tmp = path.join(path.dirname(oldAbs), `__kebab_tmp__${path.basename(oldAbs)}`);
...
for (const file of patchFiles) {
  ...
  for (const { oldStem, newStem } of stems) {
    ...
    content = content.replace(fromRel, (_, kw, q, prefix) => `${kw}${q}${prefix}${newStem}${q}`);
    const impRel = new RegExp(`(import\\s*\\(\\s*)(['"])(${relPrefix})${e}\\2(\\s*\\))`, 'g');
    content = content.replace(
      impRel,
      (_, a, q, prefix, tail) => `${a}${q}${prefix}${newStem}${q}${tail}`,
    );
    const expRel = new RegExp(`(export\\s+\\*\\s+from\\s+)(['"])(${relPrefix})${e}\\2`, 'g');
    content = content.replace(expRel, (_, kw, q, prefix) => `${kw}${q}${prefix}${newStem}${q}`);
    const expNamed = new RegExp(
      `(export\\s+(?:type\\s+)?\\{[^}]*\\}\\s+from\\s+)(['"])(${relPrefix})${e}\\2`,
      'g',
    );
    content = content.replace(expNamed, (_, kw, q, prefix) => `${kw}${q}${prefix}${newStem}${q}`);
  }
